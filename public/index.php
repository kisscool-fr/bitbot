...
$app->get('/privacy', function (Request $request, Response $response, array $args) {
    $response->getBody()->write(
        'BitBot Privacy Policy<br/><br/>It\'s just an app for fun & laugh, so we will never collect any data about you.<br/>The message you send to us are never read or analyze because the answer we send to you is completly random.<br/>The only info we have for few milliseconds is a unique user_id send by the platform needed to answer you back.<br/>After the answer is sent, we do not have the info anymore.<br/>Have fun !<br/>-----<br/>The BitBot' // phpcs:ignore
    );
    return $response;
});
